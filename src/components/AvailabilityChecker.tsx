...
export default function AvailabilityChecker() {
  ...
  const [view, setView] = useState<'calendar' | 'search'>('calendar');
  ...
  return (
    <section className="space-y-4 sm:space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="px-2">
          <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">
            {text('Chế độ xem', 'View mode')}
          </p>
        </div>

        <div className="flex flex-1 justify-end gap-1 sm:flex-none">
          <button
            type="button"
            onClick={() => setView('calendar')}
            className={`inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl px-4 text-xs font-extrabold transition sm:flex-none ${
              view === 'calendar'
          ...
          >
            ...
            {text('Tìm căn theo ngày', 'Search dates')}
          </button>
        </div>
      </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="font-display text-base font-extrabold text-slate-900 dark:text-white sm:text-lg">
                      {text('Kiểm tra căn trống Airbnb', 'Airbnb Availability Search')}
                    </h2>
